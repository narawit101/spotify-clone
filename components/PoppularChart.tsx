...
import Chart from "@/data/mokchart.json";

export default function PoppularChart() {
    ...
    return (
        <div>
            <div className='flex justify-between'>
                <a className='text-2xl hover:underline decoration-solid font-bold ml-2' href="#">ชาร์ตที่แนะนำ</a>
                ...
                <div ref={scrollRef}
                    ...
                    {Chart.map((artis: any) => (
                        <div key={artis.id} className='relative group/card flex flex-col flex-none gap-2.5 p-2 hover:bg-[#28282883] rounded-lg cursor-pointer transition'>
                            <img src={artis.image}
                                className=' flex items-center justify-center rounded-md shadow-lg aspect-square object-contain'
                                width={170}
                                height={170}
                                alt="" />
                            <div
                                className='absolute bottom-15 right-4 bg-[#1ed760] p-3 rounded-full  opacity-0 translate-y-3
                            ...
                            <div className='w-40 mt-1'>
                                <p className='font-light text-[14px] text-[#bab9b9e5] line-clamp-2'>{artis.name} </p>
                            </div>
                        </div>
                    ))}
                </div>
